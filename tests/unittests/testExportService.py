...
class TestExports(TestCase):
    ...
    def setUp(self):
        def request_callback(request, uri, headers):
            if 'data_type' not in request.parsed_body \
                    or not request.parsed_body['data_type']:
                return 200, headers, \
                    "This is a 200 response from classic, but actually no "
                "data_type was found"
            ...
            if "real" in request.parsed_body['bibcode']:
                ...
                hdr = hdr.replace(
                    '\d+',
                    str(len(request.parsed_body['bibcode']))
                )
                hdr = hdr.replace('\.', '.')
                response = '{0}{1}'.format(hdr, response)
    ...
    def test_emptypayload(self):
        """
        Ensure that an empty payload returns 400
        """
    ...
    def test_BibtexRoute(self):
        """
        test bibtex endpoint
        """
        u = url_for('bibtex')

        # bibcodes not found should return 400
        r = self.client.get(u+'?bibcode=fake')
        self.assertStatus(r, 400)
        self.assertEqual(
            r.json,
            {'error': 'No records returned from ADS-Classic'}
        )
        ...
        self.assertEqual(
            r.json,
            {'msg': 'Retrieved 1 abstracts, starting with number 1.  Total '
                    'number selected: 1.', u'export': u'bibtex response'}
        )
        ...
        self.assertEqual(
            r.json,
            {'msg': 'Retrieved 1 abstracts, starting with number 1.  Total '
                    'number selected: 1.', 'export': 'bibtex response'}
        )
    ...
    def test_EndnoteRoute(self):
        ...
        self.assertEqual(
            r.json,
            {u'error': u'No records returned from ADS-Classic'}
        )
        ...
        self.assertEqual(
            r.json,
            {u'msg': u'Retrieved 1 abstracts, starting with number 1.  Total number selected: 1.', u'export': u'endnote response'}
        )
    ...
    def test_AastextRoute(self):
        ...
        self.assertEqual(
            r.json,
            {u'msg': u'Retrieved 1 abstracts, starting with number 1.  Total number selected: 1.', u'export': u'aastex response'}
        )
